...
class TemporalAdapter(nn.Module):
    ...
    def __init__(self, sigma=0.15, input_dim=512):
        super().__init__()
    ...
    def forward(self, x):
        ...
        dist = torch.abs(idxs[None, :] - idxs[:, None])
        # === 正确的 Softmax 注意力 ===
        # 与论文一致：A = softmax( - |i-j| / sigma )
        adj = -dist / self.sigma
        adj = torch.softmax(adj, dim=-1)   # 行方向softmax

        x = torch.matmul(adj, x)           # [T,T] @ [B,T,C] → [B,T,C]
        x = self.ln(x)
        return x
'''
TH's change
...
class SKIModule(nn.Module):
    ...
    def forward(self, xt):
        ...
        x_ski = self.linear_proj(x_ski)  # 线性变换融合特征
        return x_ski
...
class OVVADModel(nn.Module):
    ...
    def forward(self, video_features):
        ...
        video_repr = (x_ta * attn).sum(dim=1)  
